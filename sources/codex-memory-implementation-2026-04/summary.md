...
这组材料显示，Codex memory 的主体实现并不在 `app-server`，而是在 `core/src/memories/` 和 `state/src/runtime/memories.rs`。`app-server` 暴露的是控制接口，例如线程级 `memory_mode` 开关和 `memory/reset`；真正的 pipeline 则是在 root session 启动时异步触发，先把满足条件的旧线程做 `Phase 1` 提取并写入 sqlite，再把这些 stage-1 输出同步成 `raw_memories.md` / `rollout_summaries/*.md`，最后起一个内部 consolidation subagent 生成 `MEMORY.md`、`memory_summary.md` 和可选 `skills/`。未来线程并不是直接读旧聊天，而是通过 `read_path.md` 模板生成的 developer instructions 来消费 `memory_summary.md`。
