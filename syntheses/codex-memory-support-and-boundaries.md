...
最近 Codex 也加了 memory 的功能了，简单看实现，主线其实很直接：

1. 先用 `rule-based` 判断哪些 `thread` 值得进入 memory 流程，比如不能太旧、也不能是刚刚还活跃的，而是要 idle 一段时间。[[sources/codex-memory-implementation-2026-04/source/local-code-reading-codex-memory-pipeline-2026-04-20|实现笔记]]
2. 再用 `rule-based` 把这些 `thread` 的 `conversation history` filter 成更适合让模型阅读的 `context`，比如去掉 `developer message`、注入进去的用户指令片段、`skill payload` 这类 prompt scaffolding。[[sources/codex-memory-implementation-2026-04/source/local-code-reading-codex-memory-pipeline-2026-04-20|实现笔记]]
3. 然后让模型对每个 `thread` 做一次 one-turn 的提炼，产出这个 `thread` 自己的 `raw memory`。[[sources/codex-memory-implementation-2026-04/source/local-followup-clarifications-codex-memory-pipeline-2026-04-20|补充澄清]] [[sources/codex-memory-implementation-2026-04/source/openai-codex-d62421d-memory-stage-one-system|stage_one_system prompt]]
4. 这些 `raw memory` 会先进入 `~/.codex` 下面的 memory 工作区，作为后续整理的原材料。[[sources/codex-memory-implementation-2026-04/source/local-code-reading-codex-memory-pipeline-2026-04-20|实现笔记]]
5. 最后在后台起另一个 consolidate `agent`，在合适的时机去整理这些 `raw memory`，更新两个文件：
   - `MEMORY.md`
   - `memory_summary.md` [[sources/codex-memory-implementation-2026-04/source/local-code-reading-codex-memory-pipeline-2026-04-20|实现笔记]] [[sources/codex-memory-implementation-2026-04/source/openai-codex-d62421d-memory-consolidation|consolidation prompt]]
6. 其中 `memory_summary.md` 是一定会进 `context` 的，更像 `MEMORY.md` 的一个 index；`MEMORY.md` 则是按需读取。[[sources/codex-memory-implementation-2026-04/source/local-followup-clarifications-codex-memory-pipeline-2026-04-20|补充澄清]] [[sources/codex-memory-implementation-2026-04/source/openai-codex-d62421d-memory-read-path|read_path prompt]]
...
- 它会直接丢掉一些明显不适合 memory 提炼的内容，例如：
  ...
  - `reasoning trace`
  - 一些内部整理或中间态 item
- 对 user message，它还会进一步删掉某些 prompt scaffolding 片段，例如注入进去的用户指令片段和 `skill payload`；这些内容不属于真正应该被记住的对话内容 [[sources/codex-memory-implementation-2026-04/source/local-code-reading-codex-memory-pipeline-2026-04-20|实现笔记]]
...
通过 `Phase 1` 之后，每个合格 `thread` 会得到一个 stage-1 output。它可以被理解成一种中间层 structured data，但这个 structured data 不是纯 JSON 对象结束就完了，而是：

- sqlite 里有结构化字段
- `raw memory` 自己又是一段有固定写作约束的 markdown 文本 [[sources/codex-memory-implementation-2026-04/source/local-code-reading-codex-memory-pipeline-2026-04-20|实现笔记]] [[sources/codex-memory-implementation-2026-04/source/openai-codex-d62421d-memory-stage-one-system|stage_one_system prompt]] [[sources/codex-memory-implementation-2026-04/source/openai-codex-d62421d-memory-stage-one-input|stage_one_input prompt]]
...
UI 里的 `Skip tool-assisted chats` 不是一个表层开关，它在实现上对应的是：当 `thread` 包含 MCP / web search 这类外部上下文时，`thread` 会被标成 `polluted`，并且在必要时触发一次全局 consolidation 去移除旧基线里受污染的 memory。[[sources/codex-memory-implementation-2026-04/source/local-code-reading-codex-memory-pipeline-2026-04-20|实现笔记]]

这说明 Codex 团队对 memory 的一个核心约束是：它宁可少记，也不想把带外部上下文污染的 `thread` 直接推进 durable memory。
...
`Codex memory 已经有一条完整但仍然初级的本地实现链路：它会筛选旧 thread，用单轮抽取生成 stage-1 outputs，再同步成 memory 工作区，并通过一个全局 consolidation subagent 维护分层的 memory 文件；未来线程默认消费的是 memory_summary 这个路由层，而不是一份无结构的长期记忆全文。` [[sources/codex-memory-implementation-2026-04/source/local-code-reading-codex-memory-pipeline-2026-04-20|实现笔记]] [[sources/codex-memory-implementation-2026-04/source/local-followup-clarifications-codex-memory-pipeline-2026-04-20|补充澄清]] [[sources/codex-memory-implementation-2026-04/source/openai-codex-d62421d-memory-consolidation|consolidation prompt]] [[sources/codex-memory-implementation-2026-04/source/openai-codex-d62421d-memory-read-path|read_path prompt]]
