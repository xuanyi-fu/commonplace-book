...
- Symphony 的机制突破是把传统 Codex `while(tool_call)` agent loop 外包上一层 `while(issue_needs_proceed)` orchestration loop：每个 Codex turn 里仍然是模型 inference / tool calls / observations 的内层循环，但 Symphony daemon 会在 issue 仍 active 时继续 nudge 下一 turn / worker session，直到 issue state 进入 noneligible / handoff / terminal 状态。[[sources/openai-codex-agent-loop-2026-01/source/unrolling-the-codex-agent-loop-markdown#^codex-agent-loop-turn-thread-history|Codex thread and turn]] [[sources/openai-codex-symphony-2026-04/source/symphony-spec#^symphony-spec-worker-continuation|worker continuation]] [[sources/openai-codex-symphony-2026-04/source/symphony-spec#^symphony-spec-app-server-thread-turn|thread reuse]]
...
- 用户提出一个更机制化的 final takeaway：之前 Codex / coding agent 的基本运行模式可以近似看作 `while(tool_call)`，模型和 coding-agent harness 都围绕一轮 turn 内的 tool-call loop 优化；Symphony 把这个节奏扩展成外层 `while(issue_needs_proceed)`。当前 coding agent 基建还没有原生跟上 issue-level long-running loop，所以 Symphony 用 daemon polling、eligible issue selection、same-thread continuation turns、short continuation retry / nudge 的方式，在现有 Codex App Server 基础设施上实现了这个外层 loop。[[sources/openai-codex-agent-loop-2026-01/source/unrolling-the-codex-agent-loop-markdown#^codex-agent-loop-turn-thread-history|Codex thread and turn]] [[sources/openai-codex-symphony-2026-04/source/symphony-spec#^symphony-spec-worker-continuation|worker continuation]] [[sources/openai-codex-symphony-2026-04/source/symphony-spec#^symphony-spec-app-server-thread-turn|thread reuse]]
...
- `issue-level agent loop`: useful concept for Symphony's outer orchestration loop: the per-turn Codex agent loop still handles tool calls, while the daemon keeps issuing continuation turns while the issue remains active and needs progress; existing page: none found yet; confidence: 0.9
