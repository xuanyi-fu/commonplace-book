...
- 每个 `agent harness` component 都是在编码一个“当前模型做不到什么”的假设，因此 harness 必须随着模型变强不断调整，删掉那些已经陈旧、不再适用于新模型的假设。[[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^harness-components-encode-model-assumptions]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^reexamine-harness-strip-and-add]]
- `planner-generator-evaluator` 模式至少对当下模型仍然受用：`planner` 假设模型容易 under-scope，需要把 raw prompt 展开成更完整的 spec；`evaluator` 假设模型 self-evaluation 容易给自己放水，需要 separate evaluator 做 skeptical review。[[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^planner-prevents-under-scoping]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^self-evaluation-bias]]
- `generator/evaluator` loop 最适合主观、复杂、能力边缘的任务，需要明确 rubrics、hard thresholds、logs 和 calibration，而不是无条件替代 single-agent + tests 的普通软件任务。[[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^frontend-harness-two-insights]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^evaluator-load-bearing-boundary]]
...
### What Comes Next Closing Lessons

- Source span label: entire `## What comes next` section, before `## Acknowledgements`
- Quoted original span or citation: [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^model-improvement-harness-frontier]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^reexamine-harness-strip-and-add]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^harness-space-moves]]
- Guiding question: 文章最后给出的 `harness` 维护原则是什么？它是否解决了前文 evidence 的边界问题？
- User recitation: 用户先质疑第一段的对比句：如果存在一个“上帝模型”能直接 sample 出任何目标产物，那 `harness` 的 development space 应该变小甚至消失，所以 “the better the models get, the more space...” 不能无条件成立。随后用户认为第二段关于 new model lands 后重新检查 harness 的话很中肯，可以概括为：stay simple, ready to delete, and push the edge forward。
- Calibrated understanding: 准确。第一段需要补一个隐含前提：模型能力提升的同时，任务 frontier 也会被推向更复杂的目标；否则在极限“上帝模型”假设下，旧 scaffold 和新 harness space 都会消失。更严谨的读法是：old scaffolding often shrinks, but the frontier for harnesses can move outward when stronger models make harder tasks reachable. 第二段是更可靠的工程结论：对真实模型跑 realistic traces，按目标调优；复杂任务有时仍能从 decomposition / specialized agents 得到收益；新模型发布后应 re-examine harness，删掉不再 load-bearing 的组件，并加上能 push capability frontier 的新组件。
- Missing points: 这段没有修复前面 V2 evidence 的实验设计问题；它更像从经验里抽出 maintenance principle，而不是严格证明 sprint removal 的 causal effect。
- Open questions: appendix 的 planner-generated spec 是否还有必要读，还是主文已经完成，可以直接总结全篇评价。
...
- 用户对 `What comes next` 的最终提炼是：第一段的 “space” 论证需要补任务 frontier 外移的前提；更中肯的是第二段的维护原则，即 stay simple, ready to delete, and push the edge forward。[[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^model-improvement-harness-frontier]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^reexamine-harness-strip-and-add]]
- 用户对全文最终总结为三条：第一，每个 `agent harness` component 都是在编码一个“当前模型做不到什么”的假设，因此 harness 必须随着模型变强不断调整，删掉已经陈旧、不再适用于新模型的假设；第二，`planner-generator-evaluator` 模式至少对当下模型仍然受用，`planner` 防 under-scope，`evaluator` 防 self-evaluation 放水；第三，`generator/evaluator` loop 最适合主观、复杂、能力边缘的任务，需要明确 rubrics、hard thresholds、logs 和 calibration，而不是无条件替代 single-agent + tests 的普通软件任务。[[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^harness-components-encode-model-assumptions]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^reexamine-harness-strip-and-add]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^planner-prevents-under-scoping]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^self-evaluation-bias]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^frontend-harness-two-insights]] [[sources/anthropic-harness-design-long-running-apps-2026-04/source/harness-design-long-running-apps-markdown#^evaluator-load-bearing-boundary]]
